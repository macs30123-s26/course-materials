...
To run a PySpark notebook in interactive mode, enter into an sinteractive session (using relevant sbatch commands; here, requesting 4 worker cores and 1 primary core -- 5 cores total, each with 8GB RAM -- in this directory for one hour):

```bash
sinteractive --time=01:00:00 --nodes=1 --ntasks=5 --mem=40G --partition=caslake --account=macs30123
```
...
pyspark --total-executor-cores 4 --executor-memory 8G --driver-memory 8G
```
